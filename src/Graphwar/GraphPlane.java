...
public class GraphPlane extends JPanel implements ActionListener {
	...
	private void initializeCurrentMarker(Graphwar graphwar, MediaTracker mediaTracker) throws Exception {
		...
		BufferedReader read = new BufferedReader(
				new InputStreamReader(graphwar.getClass().getResourceAsStream(filePath)));
	...
	private void drawBackground(Graphics2D g, boolean reversed) {
		if (reversed) {
			g.drawImage(graphwar.getGameData().getObstacle().getImage(), 0, 0, Constants.PLANE_LENGTH,
					Constants.PLANE_HEIGHT, Constants.PLANE_LENGTH, 0, 0, Constants.PLANE_HEIGHT, null);
		} else {
		...
		for (int i = 0; i < 50; i++) {
			if ((i + 5) % 10 == 0) {
				g.drawLine(Constants.PLANE_LENGTH * i / 50, Constants.PLANE_HEIGHT / 2 - 10,
						Constants.PLANE_LENGTH * i / 50, Constants.PLANE_HEIGHT / 2 + 10);
			} else if (i % 5 == 0) {
				g.drawLine(Constants.PLANE_LENGTH * i / 50, Constants.PLANE_HEIGHT / 2 - 6,
						Constants.PLANE_LENGTH * i / 50,
						Constants.PLANE_HEIGHT / 2 + 6);
			} else {
				g.drawLine(Constants.PLANE_LENGTH * i / 50, Constants.PLANE_HEIGHT / 2 - 3,
						Constants.PLANE_LENGTH * i / 50,
						Constants.PLANE_HEIGHT / 2 + 3);
			}
		...
		for (int i = 0; i < 30; i++) {
			if ((i + 5) % 10 == 0) {
				g.drawLine(Constants.PLANE_LENGTH / 2 - 10, Constants.PLANE_HEIGHT * i / 30,
						Constants.PLANE_LENGTH / 2 + 10, Constants.PLANE_HEIGHT * i / 30);
			} else if (i % 5 == 0) {
				g.drawLine(Constants.PLANE_LENGTH / 2 - 6, Constants.PLANE_HEIGHT * i / 30,
						Constants.PLANE_LENGTH / 2 + 6, Constants.PLANE_HEIGHT * i / 30);
			} else {
				g.drawLine(Constants.PLANE_LENGTH / 2 - 3, Constants.PLANE_HEIGHT * i / 30,
						Constants.PLANE_LENGTH / 2 + 3, Constants.PLANE_HEIGHT * i / 30);
			}
	...
	private void drawFunction(Graphics g, boolean terrainReversed) {
		if (graphwar.getGameData().isDrawingFunction()) {
			...
			drawFunctionImage(functionGraphics,
					(funcReversed || terrainReversed) && !(funcReversed && terrainReversed));
	...
	private Image getSoldierImage(Soldier soldier, int player) {
		if (soldier.isExploding()) {
			...
			if (timePassedAnimating < deathFadeDuration) {
				BufferedImage fadeImage = new BufferedImage(deathImages[deathImages.length - 1].getWidth(null),
						deathImages[deathImages.length - 1].getHeight(null), BufferedImage.TYPE_4BYTE_ABGR);
			...
			for (int i = 0; i < numSoldiers; i++) {
				if (soldiers[i].isAlive() || soldiers[i].isExploding()) {
					...
					if (drawImage != null) {
						...
						if ((player.getTeam() == Constants.TEAM2 || reversed)
								&& !(player.getTeam() == Constants.TEAM2 && reversed)) {
