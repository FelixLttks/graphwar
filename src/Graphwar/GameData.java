...
public class GameData implements Runnable {
	...
	public void sendFunctionPreview(String functionPreview) {
		...
		if (currentPlayer.isLocalPlayer() && drawingFunction == false) {
			try {
				String message = NetworkProtocol.FUNCTION_PREVIEW + "&" + currentPlayer.getID() + "&"
						+ URLEncoder.encode(functionPreview, "UTF-8");
				serverConnection.sendMessage(message);
			} catch (UnsupportedEncodingException e) {
				e.printStackTrace();
			}
	...
	public void sendFunction(String function) {
		...
		if (currentPlayer.isLocalPlayer() && drawingFunction == false) {
			...
			try {
				String message = NetworkProtocol.FIRE_FUNC + "&" + currentPlayer.getID() + "&"
	...
	public void angleUp() {
		if (angleUp == false && this.getCurrentTurnPlayer().isLocalPlayer()
				&& !(this.getCurrentTurnPlayer() instanceof ComputerPlayer)) {
	...
	public void angleDown() {
		if (angleDown == false && this.getCurrentTurnPlayer().isLocalPlayer()
				&& !(this.getCurrentTurnPlayer() instanceof ComputerPlayer)) {
			this.timeStartedAngle = System.currentTimeMillis();
			this.angleDown = true;
	...
	public void stopAngle() {
		if (this.getCurrentTurnPlayer().isLocalPlayer() && !(this.getCurrentTurnPlayer() instanceof ComputerPlayer)) {
			...
			String message = NetworkProtocol.SET_ANGLE + "&" + player.getID() + "&"
					+ player.getCurrentTurnSoldierIndex() + "&" + angle;
			serverConnection.sendMessage(message);
		}
	}
	...
	private void removePlayerMessage(String[] info) throws Exception {
		...
		((PreGameScreen) graphwar.getUI().getScreen(Constants.PRE_GAME_SCREEN)).addChat(null,
		...
		if (gameState == Constants.GAME) {
			...
			((GameScreen) graphwar.getUI().getScreen(Constants.GAME_SCREEN)).addChat(null,
					player.getName() + " has left the game.");
	...
	private void startGameMessage(String[] info) throws Exception {
		...
		((GameScreen) graphwar.getUI().getScreen(Constants.GAME_SCREEN)).nextTurn();
	...
	public synchronized int getCurrentFunctionPosition() {
		...
		int numDrawSteps = (int) ((System.currentTimeMillis() - timeStartedDrawingFunction)
				* Constants.FUNCTION_VELOCITY) / 1000;
		...
		if (numDrawSteps > function.getNumSteps() && drawingFunction) {
			...
			if (isFunctionReversed()) {
				obstacle.setExplosion(Constants.PLANE_LENGTH - (int) function.getLastX(), (int) function.getLastY(),
		...
		if (player.getTeam() == Constants.TEAM1) {
			switch (gameMode) {
				case Constants.NORMAL_FUNC:
					function.processFunctionRange(obstacle, players.toArray(new Player[0]), players.size(), currentTurn,
							false);
					break;
				case Constants.FST_ODE:
					function.processRK4Range(obstacle, players.toArray(new Player[0]), players.size(), currentTurn,
							false);
					break;
				case Constants.SND_ODE:
					function.processRK42Range(obstacle, players.toArray(new Player[0]), players.size(), currentTurn,
							player.getCurrentTurnSoldier().getAngle(), false);
					break;
			}
		} else {
			switch (gameMode) {
				...
				case Constants.SND_ODE:
					function.processRK42Range(obstacle, players.toArray(new Player[0]), players.size(), currentTurn,
							player.getCurrentTurnSoldier().getAngle(), true);
					break;
			}
		}
	...
	private void finishGameMessage(String[] info) throws Exception {
		...
		Thread.sleep(200); // This should be enough so that the game is not being painted when players are
							// removed
	...
	public void kickFromGame() {
		if (this.gameState == Constants.PRE_GAME) {
			((PreGameScreen) graphwar.getUI().getScreen(Constants.PRE_GAME_SCREEN))
